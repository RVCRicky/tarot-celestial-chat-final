...
export function isShiftActive(shift, date = new Date()) {
  return shift === currentShift(date)
}

export function readerShift(readerName) {
  return READERS.find((r) => r.name === readerName)?.shift || null
}
...
export function isSmallTalk(text) {
  const lower = normalizeText(text)
  return ['hola', 'buenas', 'vale', 'ok', 'gracias', 'izquierda', 'derecha', 'centro', 'si', 'sí', 'perfecto'].includes(lower)
}

export function isMainQuestion(text) {
  const lower = normalizeText(text)
  if (!lower || isSmallTalk(text)) return false
  ...
  return (
    ...
    lower.includes('como sera') ||
    lower.includes('cómo será') ||
    lower.includes('me miras') ||
    lower.includes('mirame') ||
    lower.includes('mírame') ||
...
export function isFollowup(text) {
  ...
  return (
    ...
    lower.includes('cuando llega') ||
    lower.includes('en cuanto llega') ||
    lower.includes('cuanto tarda') ||
    lower.includes('cómo será') ||
    lower.includes('como sera') ||
    lower.includes('quien es') ||
    lower.includes('qué va a pasar') ||
    lower.includes('que va a pasar') ||
    lower.includes('mira ahora') ||
    lower.includes('mirame ahora') ||
    lower.includes('mírame ahora')
  )
}

export function isSubstantiveReaderQuestion(text) {
  const lower = normalizeText(text)
  if (!lower || isSmallTalk(text)) return false
  return isMainQuestion(text) || isFollowup(text) || lower.length >= 14
}

export function estimateTypingMs(text, options = {}) {
  const {
    base = 2200,
    cps = 10.5,
    min = 1800,
  ...
  const computed = base + (len / Math.max(cps, 1)) * 1000 + extraPause
