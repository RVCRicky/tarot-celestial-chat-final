...
export default function PaymentClient() {
  ...
  const preselectedPack = useMemo(
    () => PACKS.find((pack) => pack.id === requestedPack)?.id || '',
    [requestedPack]
  )
  ...
  useEffect(() => {
    if (!preselectedPack || !autoStart) return
    const timer = setTimeout(() => {
      handleCheckout(preselectedPack)
    }, 700)

    return () => clearTimeout(timer)
  }, [preselectedPack, autoStart])
      ...
      <div style={{ maxWidth: 980, margin: '0 auto' }}>
        <div style={{ background: '#fff', borderRadius: 24, border: '1px solid #efe1bc', boxShadow: '0 12px 30px rgba(88, 41, 125, 0.08)', padding: 28 }}>
          <div style={{ textAlign: 'center', marginBottom: 24 }}>
            ...
            <p style={{ color: '#8a6a2f' }}>
              {preselectedPack
                ? 'Estamos preparando tu pasarela de pago segura...'
                : 'Elige el paquete que prefieras y pasarás a la pasarela de pago segura.'}
            </p>
          </div>

          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0, 1fr))', gap: 18 }}>
            {PACKS.map((pack) => {
              const isSelected = preselectedPack === pack.id
              return (
                <div key={pack.id} style={{ background: '#faf7ff', border: isSelected ? '2px solid #d6ad45' : '1px solid #ece1ff', borderRadius: 20, padding: 20, display: 'flex', flexDirection: 'column', justifyContent: 'space-between', minHeight: 250 }}>
                  <div>
                  ...
                  <button onClick={() => handleCheckout(pack.id)} disabled={loadingPack === pack.id} style={{ marginTop: 14, padding: '14px 16px', borderRadius: 14, border: 'none', background: '#6f3ea8', color: '#fff', fontWeight: 800, cursor: 'pointer' }}>
                    {loadingPack === pack.id ? 'Conectando...' : isSelected && autoStart ? 'Preparando cobro...' : 'Comprar ahora'}
                  </button>
                </div>
              )
            })}
